...
class HashTable {
  ...
  insertWithHashCollisions(key, value) {
    ...
    if (!this.data[index]) {
      ...
      //bucket is occupied -- add to head
  ...
  insert(key, value) {
    ...
    // iterate to check if same key already exists
    let currentPair = this.data[index] // null?
    ...
    if (currentPair) {
      ...
      // inserting at index, equivalent to add to head
      if (!this.data[index]) {
        this.data[index] = newPair
      } else {
        newPair.next = this.data[index]
