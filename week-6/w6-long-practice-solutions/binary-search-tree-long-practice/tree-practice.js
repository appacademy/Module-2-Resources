...
function findMinBSTIterative (rootNode) {
  //!!START
  while (rootNode.left) {
...
function findMaxBSTIterative (rootNode) {
  //!!START
  while (rootNode.left) {
    rootNode = rootNode.left;
  }
...
function findMinBTIterative (rootNode) {
  //!!START
  let min = rootNode.val;
...
function findMaxBTIterative (rootNode) {
  //!!START
  let max = rootNode.val;
...
function getHeightIterative (rootNode) {
  //!!START
  if (!rootNode) return -1;
  let height = 0;
  rootNode.level = height;
  ...
  while (queue.length) {
    ...
    if (curr.level > height) height = curr.level;
