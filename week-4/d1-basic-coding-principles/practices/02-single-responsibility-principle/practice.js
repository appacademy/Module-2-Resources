...
function bakePies(pieQuantity, pieType, recipe) {
  for (let i = 0; i < pieQuantity; i++) {
    ...
    // combiningMsg += recipe.map(ingredient => ingredient.name).join(', ');
    // console.log(combiningMsg);
    combineIngredients(recipe, pieType);
...
function sellPies(costOfPie, profitMargin, pieQuantity) {
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin=1.2) {
  ...
  // console.log(`Sold ${pieQuantity} pies for $${revenue.toFixed(2)}!`);
  sellPies(costOfPie, profitMargin, pieQuantity);
}
